$(document).ready(function () {
  ...
  //scroll from btn__price to form bottom:focus 
    $("a[href^='#']").on("click", function(event){
      event.preventDefault();      
      var self = $(this);
      $.scrollTo($(this).attr("href"), 1000, {
        onAfter: function(){//callback            
          if(self.attr("href") == "#form__invite"){
            $("#form__invite").focus();
          }
        }
      });      
    });
  //Маска номера телеонфа
  ...
  function schet() {
    console.log($(".range__slider").val());
      ...
      $('#colz').html(XFormatPrice(col) + " руб.");
      $('#marg').html(XFormatPrice(vir) + " руб.");
      $('#rek').html(XFormatPrice(rek) + " руб.");
      $('#seb').html(XFormatPrice(seb) + " руб.");
      $('#fond').html(XFormatPrice(fond) + " руб.");
      $('#nal').html(XFormatPrice(nal + " руб."));
      $('.calculation__result-val').html(XFormatPrice(clear) + " руб.");
  }                
